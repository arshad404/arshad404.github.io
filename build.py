...
def parse_post(path):
    ...
    slug = path.stem
    return {'title': title_match.group(1).strip(), 'year': year, 'topic': tags[0][1:] if tags else 'journal', 'tags': tags, 'excerpt': plain[:180] + ('...' if len(plain) > 180 else ''), 'read': f'{max(1, (len(plain.split()) + 199) // 200)} min read', 'html': markdown_to_html(body), 'url': f'year/{year}/{slug}.html'}
...
<body><main><a class="back" href="../../index.html">← Back to all writing</a><div class="meta"><span>{post['year']} / {html.escape(post['topic'])}</span><span>{post['read']}</span></div><h1>{html.escape(post['title'])}</h1><article>{post['html']}</article></main></body></html>
''', encoding='utf-8')
